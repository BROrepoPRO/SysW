...
STEP_TIMEOUTS = {
    ...
    "vbacompile": 60,
    "templates": 600,
    # Миграция SysW.db из base/models/* — долгий этап (БД крупная: сотни тысяч
    # строк в parts_catalog/parts). Фактическое время ~5.5-6 мин, поэтому таймаут
    # 300с недостаточен и приводил к ложному [ТАЙМАУТ]. Увеличено до 600с.
    "migrate": 600,
    # Этап run_tests (макрос RunAllTests в work.xlsm) — синхронный COM-вызов,
    # фактически выполняется ~60-65 мин (тяжёлые тесты AutoMatch/GetWorks по БД
    # с ~675 тыс. записей в parts_catalog и ~4 млн в parts). Таймаут 900с (15 мин)
    # приводил к ложному [ТАЙМАУТ] и принудительному taskkill легитимно
    # работающего этапа (валидировано по logs/test_results.log: в успешном прогоне
    # шаг [3/5]->[4/5] занял ~65 мин). Увеличено до 5400с (90 мин) — v1.0.9.
    "tests": 5400,
}
