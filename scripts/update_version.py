...
# Регулярное выражение для проверки формата версии (SemVer: X.Y.Z)
VERSION_RE = re.compile(r"^\d+\.\d+\.\d+$")


def validate_version(version):
    """Проверяет, что версия соответствует формату X.Y.Z (SemVer)."""
    if not VERSION_RE.match(version):
        print(f"ОШИБКА: Некорректный формат версии '{version}'. "
              f"Ожидается формат X.Y.Z (например, 1.1.0).")
        sys.exit(1)
...
def main():
    ...
    parser.add_argument(
        "version",
        help="Новая версия в формате X.Y.Z (например, 1.1.0).",
    )
