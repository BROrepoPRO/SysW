...
def main():
    ...
    try:
        ...
        # v1.0.9: FreezePanes A4 для модельных файлов base/models/*.xlsm.
        ...
        # ВАРИАНТ C (исправление v1.0.9): применяется на XML-уровне (правка только
        # <pane> в sheet*.xml через zip), т.к. прежний openpyxl (keep_vba=True) делал
        # модели нечитаемыми для Excel COM (валидировано: -2147352567). Функция
        # идемпотентна — повторные прогоны не трогают уже закреплённые файлы.
