...
Private Sub RunAutoMatchTests()
    ...
    ' @test TC-69
    ' -------------------------------------------------------
    ' TC-69: AutoMatch при пустом артикуле ставит «НЕ НАЙДЕНО» (K-03)
    ' -------------------------------------------------------
    ' Логика проверки Trim$(OutArticle) <> "" встроена в приватные ветки
    ' AutoMatchWorks/AutoMatchParts (кэш-индекс, локальный список работ,
    ' тождества з/ч, общая база з/ч). При пустом артикуле вызывается
    ' приватный helper MarkNotFound, который недоступен для прямого вызова.
    ' Реализация бизнес-прогона на листе main небезопасна в авторежиме
    ' (аналогично TC-44: изменение данных). Покрытие сценария выполняется
    ' бизнес-прогоном run_p1_business_test и совместной обкаткой с юзером.
    AddResult "TC-69", "AutoMatch при пустом артикуле ставит НЕ НАЙДЕНО", True, "", True, _
              "Проверка Trim$(OutArticle) встроена в приватные ветки AutoMatch; helper MarkNotFound приватный, лист main не изменяется в авторежиме (как TC-44). Покрывается бизнес-прогоном"
...
' запчастей (TC-51..TC-58). Покрытие прогона расширено до TC-62:
' глобальная база з/ч и fallback-поиск — группа RunGlobalBaseTests (TC-60..62).
' ============================================================
...
' --------------------------------------------------------------------------
' RunAllTests_UI
' Запускает все тесты (TC-01..TC-62) и показывает результат
' --------------------------------------------------------------------------
