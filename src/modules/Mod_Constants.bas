...
' ============================================================
Public Const APP_VERSION As String = "1.1.6.2"
...
' Имя файла системного лога (общие системные события)
Public Const LOG_FILE As String = "log.txt"

' Имя файла тестового лога (детальный лог тестов, уровни INFO/WARN/ERROR)
Public Const TEST_LOG_FILE As String = "test_results.log"
...
'         Запчасти: сначала по № кат. X(24); если пуст ИЛИ нет MOD_PART —
'         fallback по наименованию Y(25). Работы: только по наименованию L(12).
'         При множественных совпадениях берётся первая запись нужного типа.
' False — поведение импорта идентично прежнему.
' ============================================================
' В VBA нельзя задать значение по умолчанию прямо в объявлении модульной
' переменной, поэтому флаг выставляется в True в начале ImportDataToMain
' (см. Mod_Import.bas). Здесь только объявление (по умолчанию False).
