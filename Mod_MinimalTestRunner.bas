...
' Назначение: Автоматический прогон тестов TC-01..TC-09
' ============================================================

' Запускает основной цикл тестов
Public Sub RunAllTests()
    ...
    ' TC-03: FormatDateSQL корректной даты
    TotalTests = TotalTests + 1
    If Test_FormatDateSQL_Valid() Then
        PassedTests = PassedTests + 1
        Debug.Print "  [OK] TC-03: FormatDateSQL корректной даты"
    Else
        FailedTests = FailedTests + 1
        Debug.Print "  [FAIL] TC-03: FormatDateSQL корректной даты"
    End If
    ...
    ' Итог
    Debug.Print "========================================="
    Debug.Print "Итого: " & TotalTests & " тестов, " & PassedTests & " пройдено, " & FailedTests & " провалено"
    Debug.Print "========================================="

    MsgBox "Тестирование завершено." & vbCrLf & _
           "Итого: " & TotalTests & ", пройдено: " & PassedTests & ", провалено: " & FailedTests, _
...
' TC-08: FindOrder с существующим заказом (по № п/п = "1")
Private Function Test_FindOrder_Existing() As Boolean
    On Error Resume Next
    Dim Header As OrderHeader
    ' Ищем по № п/п (колонка A листа spisok)
