...
Проект построен на модульной архитектуре VBA с разделением на 13 стандартных модулей (`.bas`), 6 классов (`.cls`) и 4 класса листов (`.cls`). Исходный код организован в директории `src/`:
...
7. **Запустить тесты:**
   ...
   ```
   Дополнительные CLI-опции (R-16, v1.1.1.1): `--module <имя>` — выборочный запуск,
   `--verbose` — расширенный вывод, `--output <путь>` — генерация отчётов JSON/HTML.
...
- [**Реестр макросов, скриптов и тестов**](docs/DEVELOPER.md#приложение-d-реестр-макросов-vba) — полный реестр процедур VBA, скриптов автоматизации и тестов с замечаниями аудита (Приложения D–G в `docs/DEVELOPER.md`)
