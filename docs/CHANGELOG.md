...
### Added
- **Зарегистрирована отложенная глобальная задача `G-REESTR`** (Задача 13, v1.1.0): интеграция
  `docs/reestr.md` в общую документацию с последующим удалением/переносом — внесена в
  `docs/ROADMAP.md` §11; исполнение — отдельным шагом.
...
- **Скрипт мониторинга длительных процессов `scripts/monitor_long.ps1`** (Задача 11, v1.1.0)
  ...
  закреплённый лог-файл. Добавлено правило `[U6]` в `.ycarules`.

### Changed
- **Расширены правила `[E3]`/`[U4]`** (Задача 6, v1.1.0): в перечень критических файлов добавлены
  ключевые файлы системы (`scripts/*.py`, `scripts/*.ps1`, `src/modules/*.bas`,
  `src/classes/*.cls`, `src/sheets/*.cls`, `.sourcecraft`, `.vscode/settings.json`);
  `[U4]` дополнен объединением последовательностей команд Python/PowerShell в одну точку
  подтверждения. Синхронизировано в `.sourcecraft` (critical).
- **Правило `[Z8]` «Заблокированный путь → запрос пользователю»** (Задача 5, v1.1.0) внесено
  в `.ycarules`; синхронизировано в `docs/sourcecraft-guide.md`.
- **Глобальное поведение агентов (Задача 12, v1.1.0)**: вместо обязательного создания плана
  на промт — **регистрация задачи в todo** (правило `[U1]` обновлено; синхронизировано в
  `docs/sourcecraft-guide.md`, раздел «Процесс разработки»). План в `plans/` создаётся только
  для сложных/архитектурных изменений по решению Orchestrator.
