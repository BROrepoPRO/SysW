...
- [ ] ⬜ **Сделать бэкап `work.xlsm`** — скопировать файл в `_backup/work_YYYY-MM-DD.xlsm`
- [ ] ⬜ **Сделать бэкап `report.xlsx`** — скопировать файл в `_backup/report_YYYY-MM-DD.xlsx`
- [ ] ⬜ **Запустить `export_vba.py`** — убедиться, что текущее состояние VBA-модулей экспортировано на диск
- [ ] ⬜ **Закоммитить текущее состояние** — `git add -A && git commit -m "chore: backup before modernization phase 1"`

---

## Фаза 1 — Критические исправления (3.5 ч)

> **Цель:** устранить проблемы, которые блокируют работу или могут привести к потере данных.
...
- [ ] ⬜ Найти модуль `Mod_FullTestRunner`
- [ ] ⬜ Скопировать весь текст модуля
- [ ] ⬜ Вставить в файл `Mod_FullTestRunner.bas` (UTF-8 без BOM)
- [ ] ⬜ Проверить, что русские комментарии отображаются корректно
- [ ] ⬜ Запустить `python run_tests.py` — убедиться, что тесты проходят

### 1.2. Добавить 4 модуля в скрипты экспорта/импорта (0.5 ч)

- [ ] ⬜ Открыть `export_vba.py`
- [ ] ⬜ Добавить в список `COMPONENTS`:
  - `Mod_SheetButtons.bas`
  - `Mod_MainButtons.bas`
  - `Sheet_z4.cls`
  - `Sheet_work.cls`
- [ ] ⬜ Открыть `import_all_vba.py`
- [ ] ⬜ Добавить в список `FILES` те же 4 файла
- [ ] ⬜ Запустить `python export_vba.py` — убедиться, что модули экспортируются
- [ ] ⬜ Проверить `git status` — файлы должны появиться в изменениях

### 1.3. Исправить утечку ресурсов в `SearchSheetByGRZ` (0.5 ч)

- [ ] ⬜ Открыть `Mod_Import.bas`
- [ ] ⬜ Найти функцию `SearchSheetByGRZ` (строка ~47)
- [ ] ⬜ Добавить `wbReport.Close False` перед `Exit Function` (строка ~79)
- [ ] ⬜ Убедиться, что книга закрывается и при нормальном выходе, и при ошибке
- [ ] ⬜ Проверить, что `Btn_main_Import` в `Mod_MainButtons.bas` не использует открытую книгу после вызова `SearchSheetByGRZ`

### 1.4. Исправить `isProcessing` в `Sheet1_main.cls` (0.5 ч)

- [ ] ⬜ Открыть `Sheet1_main.cls`
- [ ] ⬜ Найти обработчик `Worksheet_Change`
- [ ] ⬜ Добавить обработку ошибок, чтобы `isProcessing` сбрасывался в `False` при любом исходе:
  ```vba
  ...
  ```
- [ ] ⬜ Проверить, что при ошибке в `FillHeaderFromOrder` флаг сбрасывается

### 1.5. Заменить диапазон `A2:XFD` на `B2:ZZ` (0.5 ч)

- [ ] ⬜ Открыть `Mod_Import.bas`
- [ ] ⬜ Найти `ClearMainSheet_UI` (строка ~253)
- [ ] ⬜ Заменить `"A2:XFD" & lastRow` на `"B2:ZZ" & lastRow`
- [ ] ⬜ Открыть `Mod_MainButtons.bas`
- [ ] ⬜ Найти `Btn_main_Import` (строка ~50)
- [ ] ⬜ Убедиться, что там тоже используется `"B2:ZZ"` (а не `"A2:XFD"`)

### 1.6. Устранить дублирование `Btn_main_Import` (1.5 ч)

- [ ] ⬜ Открыть `Mod_MainButtons.bas`
- [ ] ⬜ Найти `Btn_main_Import` (строка ~81)
- [ ] ⬜ Сравнить логику с `Mod_Import.ImportSheet` (строка ~131)
- [ ] ⬜ Удалить дублирующийся код из `Btn_main_Import`
- [ ] ⬜ Заменить на вызов `Mod_Import.ImportSheet`
- [ ] ⬜ Проверить, что все 8 этапов импорта сохранились
- [ ] ⬜ Протестировать кнопку импорта на листе main

### ⬜ Фаза 1 — Проверка

- [ ] ⬜ Запустить `python run_tests.py` — все тесты проходят
- [ ] ⬜ Проверить импорт из `report.xlsx` — данные корректно загружаются
- [ ] ⬜ Проверить очистку листа — очищается только B2:ZZ, не A1
- [ ] ⬜ Проверить заполнение шапки — B3:B15 заполняется корректно
- [ ] ⬜ Закоммитить: `git add -A && git commit -m "feat: phase 1 — critical fixes (encoding, export, leak, dedup)"`
...
- [ ] ⬜ Открыть `run_tests.py`
- [ ] ⬜ Заменить все строки на транслите на русский язык:
  - `ZAPUSK TESTOV` → `ЗАПУСК ТЕСТОВ`
  - `Sozdanie COM-obekta` → `Создание COM-объекта`
  - И т.д.
- [ ] ⬜ Проверить, что скрипт работает после изменений

### 3.4. Обновить документацию (1 ч)

- [ ] ⬜ Открыть `docs/DEVELOPER.md`
- [ ] ⬜ Добавить описание новых модулей:
  - `Mod_Constants.bas` — константы и типы
  - `Mod_SheetOps.bas` — операции с листами
  - `Mod_Logger.bas` — логирование
  - `Mod_Selection.bas` — подбор
- [ ] ⬜ Обновить схему взаимодействия модулей
- [ ] ⬜ Открыть `README.md`
- [ ] ⬜ Обновить структуру проекта (новые модули)
- [ ] ⬜ Открыть `CHANGELOG.md`
- [ ] ⬜ Добавить запись о фазах 1-3
...
- [ ] ⬜ Закоммитить: `git add -A && git commit -m "feat: phase 3 — selection module, cleanup, docs update"`
...
| Фаза 3 — Средний приоритет | 4 | 5.5 | ⬜ |
| Фаза 4 — Будущее | 4 | — | ⬜ |
| **Итого** | **25** | **27.0** | **⬜ 0%** |
...
| Скрипт импорта VBA | [`import_all_vba.py`](import_all_vba.py) |
