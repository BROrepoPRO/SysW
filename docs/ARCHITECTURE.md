...
>
> **Реестр макросов, скриптов и тестов:** актуальный состав процедур VBA и скриптов автоматизации приведён в [`docs/DEVELOPER.md`](DEVELOPER.md), Приложения D–F (Реестр макросов VBA, Реестр скриптов, Реестр тестов) и Приложение G (ошибки, расхождения, рекомендации).
>
> **Правила рабочей области:** единый справочник [`.ycarules`](../.ycarules) (8 модулей `[O][G][Z][K][U][S][T][E]`) и зеркальный каталог рабочих правил [`.codeassistant/rules/`](../.codeassistant/rules/) (8 файлов-зеркал). Иерархия источников правил — правило `[E5]`.
...
> **v1.0.13.** Глобальный каталог запчастей (650 000+ позиций) **вынесен из модельных книг**
> в отдельный файл `base/models/z4.xlsx`. В файле группы лист `z4` остаётся лишь **фрагментом**
> глобального каталога, актуальным для текущей `GroupName` (до 50 000 строк). Автопоиск при
> отсутствии тождества обращается к глобальному файлу через `Mod_ModelDB.ReadGlobalPartByKey`
> (fallback в `Mod_AutoMatch.AutoMatchParts`). Сборка файла — `scripts/build_global_parts.py`.
...
### 4.5. Изменения в `Лист2` (класс листа `main`)
