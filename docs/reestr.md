...
|---|---|---|---|
| `Btn_main_Clear_Click` | Очистка данных main B4:ZZ (с подтверждением) | «Очистить» | Mod_ButtonDispatcher |
| `Btn_main_Import_Click` | Импорт листа из report.xlsx по ГРЗ из B6 | «Импорт» | Mod_ButtonDispatcher |
| `Btn_main_FillHeader_Click` | Заполнение шапки B5:B17 по № из B4 | «Заполнить шапку» | Mod_ButtonDispatcher |
| `Btn_main_ClearHeader_Click` | Очистка шапки B5:B17 | «Очистить шапку» | Mod_ButtonDispatcher |
| `Btn_main_ImportByInput_Click` | Импорт по ГРЗ через InputBox | «Импорт по ГРЗ» | Mod_ButtonDispatcher |
| `Btn_main_RunTests_Click` | Запуск всех автотестов (TC-01..TC-50+TC-S*) | «Тесты» | Mod_ButtonDispatcher |
| `Btn_main_WriteLog_Click` | Запись сообщения в лог через InputBox | «Лог» | Mod_ButtonDispatcher |
| `Btn_main_RenameSheets_Click` | Переименование листов report.xlsx по ГРЗ | «Переименовать» | Mod_ButtonDispatcher |
...
| `Btn_main_ShowWorkbookPath_Click` | Показ пути к книге | «Путь к книге» | Mod_ButtonDispatcher |
| `Btn_main_ShowCurrentUser_Click` | Показ имени пользователя Windows | «Пользователь» | Mod_ButtonDispatcher |
...
| `Btn_main_ImportFromSheetM_Click` | Импорт с готового листа {B4}M без report.xlsx | «Импорт с листа M» | Mod_ButtonDispatcher |
...
### 1.3 Поиск листов UAZ и запчастей (`Mod_SheetButtons`)

| Имя макроса | Краткое описание функционала | Кнопка (если есть) | Модуль |
|---|---|---|---|
| `Btn_UAZ_Article_Click` → `Btn_UAZ_SearchByArticle` | Поиск «содержит» по ст. B | «Поиск по артикулу» | Mod_SheetButtons |
| `Btn_UAZ_Name_Click` → `Btn_UAZ_SearchByName` | Поиск по ст. C | «Поиск по наименованию» | Mod_SheetButtons |
| `Btn_UAZ_Clear_Click` → `Btn_UAZ_ClearFilter` | Сброс фильтра | «Сброс» | Mod_SheetButtons |
| `Btn_Parts_Article_Click` → `Btn_Parts_SearchByArticle` | Поиск по ст. B (z4/*z4) | «Поиск по артикулу» | Mod_SheetButtons |
| `Btn_Parts_Name_Click` → `Btn_Parts_SearchByName` | Поиск по ст. C | «Поиск по наименованию» | Mod_SheetButtons |
| `Btn_Parts_Clear_Click` → `Btn_Parts_ClearFilter` | Сброс фильтра | «Сброс» | Mod_SheetButtons |

> Приватные помощники модуля `Mod_SheetButtons` (в состав реестра включаются для полноты):
> `ExecuteUAZSearch(searchColumn)`, `ExecutePartsSearch(searchColumn)`, `IsSearchableSheet(ws)`,
> `IsPartsSheet(ws)`. Других обработчиков в модуле нет.
...
| `ImportSheet_UI` | Импорт из report.xlsx по ГРЗ из B6 | «Импорт» | Mod_Import |
| `ImportByInput_UI` | Импорт по ГРЗ через InputBox | «Импорт по ГРЗ» | Mod_Import |
| `RenameSheets_UI` | Переименование листов report.xlsx по ГРЗ | «Переименовать» | Mod_Import |
...
| `ImportFromSheetM_UI` | Импорт с готового листа {B4}M без report.xlsx | «Импорт с листа M» | Mod_Import |
...
> Набор автоматических тестов покрывает **TC-01..TC-50** + **TC-S1..TC-S3**.
