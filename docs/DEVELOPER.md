...
- **Утилиты** ([`Mod_Utils.bas`](../Mod_Utils.bas)) — вспомогательные функции, общие типы данных
- **Шапка заказа** ([`Mod_OrderHeader.bas`](../Mod_OrderHeader.bas)) — заполнение заголовка заказ-наряда
- **Импорт** ([`Mod_Import.bas`](../Mod_Import.bas)) — импорт данных из Excel в SQLite и обратно
- **Диспетчер кнопок** ([`Mod_ButtonDispatcher.bas`](../Mod_ButtonDispatcher.bas)) — прослойка между UI и бизнес-логикой
- **Тестовый раннер** ([`Mod_FullTestRunner.bas`](../Mod_FullTestRunner.bas)) — автоматическое тестирование
- **Лист main** ([`Sheet1_main.cls`](../Sheet1_main.cls)) — обработчик событий листа
...
**Файл:** [`Mod_Utils.bas`](../Mod_Utils.bas) (156 строк)
...
**Файл:** [`Mod_OrderHeader.bas`](../Mod_OrderHeader.bas) (203 строки)
...
**Файл:** [`Mod_Import.bas`](../Mod_Import.bas) (370 строк)
...
**Файл:** [`Mod_ButtonDispatcher.bas`](../Mod_ButtonDispatcher.bas) (121 строка)
...
**Файл:** [`Mod_FullTestRunner.bas`](../Mod_FullTestRunner.bas) (564 строки)
...
**Файл:** [`Sheet1_main.cls`](../Sheet1_main.cls) (40 строк)
...
|------|-----|-----------|------------|
| **Диск** | VS Code, Git, code review | **UTF-8** (без BOM) | [`export_vba.py`](../export_vba.py) |
| **Excel** | VBA Editor, COM-автоматизация | **Windows-1251** (CP1251) | [`import_all_vba.py`](../import_all_vba.py) |
...
**Файл:** [`export_vba.py`](../export_vba.py) (174 строки)
...
python export_vba.py --dry    # Просмотр без реального экспорта
```

### 4.2 import_all_vba.py

**Файл:** [`import_all_vba.py`](../import_all_vba.py) (281 строка)
...
- **Зависимости:**
  ...
  pip install pywin32
  ```
  `pywin32` требуется для COM-автоматизации Excel (используется в `export_vba.py` и `import_all_vba.py`)
...
|--------|-----------|-----------|
| [`export_vba.py`](../export_vba.py) | Выгрузка VBA из Excel на диск (CP1251 → UTF-8) | UTF-8 |
| [`import_all_vba.py`](../import_all_vba.py) | Загрузка VBA с диска в Excel (UTF-8 → CP1251) | UTF-8 |
