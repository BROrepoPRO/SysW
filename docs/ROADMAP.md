...
> **Версия системы:** 1.0.14
> **Дата:** 2026-08-26
> **Статус:** Актуализирован (**v1.0.14**): финализация релиза — актуализация документации,
> перенос `docs/reestrOLD.md` в `plans/_archive/`, очистка временных артефактов; тесты
> расширены до TC-62 (группа `RunGlobalBaseTests`: глобальная база з/ч `z4.xlsx` и
> fallback-поиск). Ранее (v1.0.12): универсальный поиск листов — имя группы из `main!$B$14`,
> нейтральные имена `Btn_Search_*`/`Btn_ClearFilter`; ручной подбор запчастей `PickParts_UI`;
...
Система SysW (v1.0.14) — модульная VBA-надстройка для Excel с обвязкой на Python/PowerShell, автоматизирующая обработку заказ-нарядов авторемонта (импорт данных из отчётов, заполнение шапки заказа, поиск по ГРЗ, учёт работ и запчастей).
...
| **Тестирование** | ✅ v0.2.0, актуализировано v1.0.14 | `Mod_FullTestRunner` — TC-01..TC-62 + TC-S1..TC-S3 (полный перечень — см. `docs/reestr.md`, раздел 3) |
