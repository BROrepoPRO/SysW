...
> **Версия системы:** 1.1.2.1
> **Дата:** 2026-08-26
> **Статус:** Актуализирован (**v1.1.2**): верификация шапки заказа после импорта
> (`scripts/run_p1_business_test.py` — `snapshot_order_header`, сводка `[ШАПКА]`, поля `B5:B17`);
> интеграционное тестирование `ImportFromB2_UI` (группа `RunImportB2IntegrationTests`,
> TC-65..TC-68) — задача **R-IT** выполнена; прогон `run_tests.py`: Total=69/Passed=60/Failed=0/
> Skipped=9. Ранее (v1.1.1): фикс зависания `AutoMatchWorks`, дефект кириллицы R-16,
> SQLite-таймаут; (v1.1.0): логгер «2 лога», версионность X.Y.Z.W, интеграция реестра.

---

## 1. Текущее состояние

Система SysW (v1.1.2) — модульная VBA-надстройка для Excel с обвязкой на Python/PowerShell, автоматизирующая обработку заказ-нарядов авторемонта (импорт данных из отчётов, заполнение шапки заказа, поиск по ГРЗ, учёт работ и запчастей).
