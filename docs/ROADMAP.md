...
> **Версия системы:** v1.1.6.2
> **Дата:** 2026-09-02
> **Статус:** Актуализирован (**v1.1.6.2**): реорганизация `.ycarules` в 8 тематических
> модулей `[O][G][Z][K][U][S][T][E]`; создан каталог рабочих правил `.codeassistant/rules/`
> (8 файлов-зеркал); зарегистрирована глобальная задача `G-CHAT-UI`. Ранее (v1.1.4):
> интеграция точного описания режимов SourceCraft Code Assistant в `.ycarules` и
> `docs/sourcecraft-guide.md`; полная миграция всех групп моделей в `matlib_entries`;
> бизнес-прогон `run_oab_reconcile_business_test.py` — **PASS** (PASS=2, FAIL=0). Ранее (v1.1.2):
> верификация шапки заказа, интеграционное тестирование `ImportFromB2_UI` (R-IT),
> прогон `run_tests.py`: Total=69/Passed=60/Failed=0/Skipped=9; (v1.1.1): фикс зависания
> `AutoMatchWorks`, дефект кириллицы R-16, SQLite-таймаут; (v1.1.0): логгер «2 лога»,
> версионность X.Y.Z.W, интеграция реестра.

---

## 1. Текущее состояние

Система SysW (v1.1.6.2) — модульная VBA-надстройка для Excel с обвязкой на Python/PowerShell, автоматизирующая обработку заказ-нарядов авторемонта (импорт данных из отчётов, заполнение шапки заказа, поиск по ГРЗ, учёт работ и запчастей).
...
| **Тестирование**                                                  | ✅ v0.2.0, актуализировано v1.1.2 | `Mod_FullTestRunner` — TC-01..TC-64 + TC-S1..TC-S3 + TC-65..TC-68 (Total=69; полный перечень — см. `docs/DEVELOPER.md`, Приложение F)                                                                                                     |
...
| **SourceCraft интеграция**                                          | ✅ v0.3.0, актуализировано v1.1.6.2 | `.ycarules` (8 модулей `[O][G][Z][K][U][S][T][E]`), зеркала `.codeassistant/rules/`, `docs/sourcecraft-guide.md`, задача `G-CHAT-UI`                                                                                                                                                                     |
...
| **R-IT** | **Интеграционное тестирование ImportFromB2_UI**   | **P1**       | TC-65..TC-68: сквозной сценарий «импорт → шапка заказа → работы (L:N) → запчасти (X:AA)» через `ImportFromB2_UI`, отсутствие исключений/зависаний |
...
| Интеграционное тестирование ImportFromB2_UI                | P1                 | ~3.0            | 1 (R-IT)                    | ✅ Выполнено (TC-65..TC-68) |
...
- [X] **R-16** · выполнено в v1.1.1.1 · независимая · без условий — расширение `run_tests.py` (`--module`, `--verbose`, `--output`, генерация отчётов JSON/HTML); подтверждено сквозным прогоном с флагами (exit 0, отчёты сгенерированы, кириллица корректна)
...
- [x] **R-IT** · выполнена (v1.1.2) · независимая · условие: наличие рабочей книги/отчёта — интеграционное тестирование `ImportFromB2_UI` (TC-65..TC-68: сквозной конвейер импорта → шапка → работы L:N → запчасти X:AA)

> **Обкатка на реальных данных (v1.1.1.1):** выполнены фикс зависания `AutoMatchWorks`
> (`OpenModelGroupFile` — защитные параметры вокруг `Workbooks.Open`) и защита от бесконечного
> ожидания при блокировке `SysW.db` (SQLite-таймаут: `ConnectionTimeout`/`CommandTimeout = 15`
> в `Mod_SQLiteDB.cls`) — относятся к устойчивости/обкатке. Прочие задачи P0/P1
> (R-08..R-12, R-14, R-15, R-17, R-WS/R-ZS/R-DS, R-IT) переносятся в следующий релиз
> без изменения статусов (по scope).
...
- [ ] **R-PUSH** · текущая (финализация v1.0.14) · независимая · условие: доступ к github.com (сеть/VPN) — выполнить `git push origin dev` (коммит `c9fb3d1`); после успешного пуша — слияние `dev → main` по подтверждению пользователя и возврат на `dev` (отдельный git-этап Orchestrator)
...
> Регистрация рабочих задач плана [`plans/_archive/promt1.1.0_plan.md`](../plans/_archive/promt1.1.0_plan.md). Статусы `[x]`/`[ ]` отражают фактическое исполнение по коду/файлам на дату актуализации. Отложенные глобальные цели (Задачи 7, 8, 13) — см. раздел «Глобальные цели верхнего уровня» (`G-COLOR`, `G-UBUNTU`, `G-REESTR`).
...
- [X] **V-1.1.4-MODES** · выполнено в v1.1.4 · независимая · без условий — интеграция точного
  описания режимов SourceCraft Code Assistant (`Code`/`Ask`/`Architect`/`Debug`/`Orchestrator`)
  в `.ycarules` (раздел 1.1) и `docs/sourcecraft-guide.md`
- [X] **V-1.1.4-MATLIB** · выполнено в v1.1.4 · независимая · без условий — полная миграция всех
  групп моделей (`2170`, `2180`, `2190`, `4x4`, `GAZ`, `UAZ`) в `matlib_entries` (SysW.db)
  через `scripts/migrate_models_to_sqlite.py`; каталог `parts_catalog` = 25 154 позиции,
  `matlib_entries` = 387 записей
- [X] **V-1.1.4-OAB** · выполнено в v1.1.4 · независимая · без условий — бизнес-прогон
  `scripts/run_oab_reconcile_business_test.py`: сверка глубокой подстановки O/AB против
  `matlib_entries` — **PASS** (PASS=2, FAIL=0), группа `GAZ`

### Известные проблемы / техдолг (итерация 1.1.2)

> НАБЛЮДЕНИЯ из бизнес-теста (`run_p1_business_test.py`, отчёт `logs/p1_test_report.log`).
> Задокументированы как известные проблемы и **не исправлялись** в итерации 1.1.2.
> Рекомендации — для последующих итераций.

- [ ] **K-01** · техдолг · зависит от анализа импорта · без условий — **номер заказа `B4` пуст
  после импорта**: до импорта был `100`, шапка `B5:B17` заполняется корректно, но сама ячейка
  `B4` очищается в процессе импорта. Рекомендация: проверить логику очистки/переноса B4
  в `Mod_Import.ImportFromB2_UI`.
- [ ] **K-02** · техдолг (производительность) · независимая · без условий — **`AutoMatchWorks`
  медленный**: фикс зависания сработал (без зависания), но время выполнения ~2 ч 46 мин на
  1 работе. Рекомендация: оптимизация производительности (кэширование/поиск по индексу
  модельных артикулов).
- [X] **K-03** · выполнено (v1.1.5) · **`AutoMatchWorks`/`AutoMatchParts` не подобрали
  артикулы** — исправлено: пустая ячейка заменена маркером «НЕ НАЙДЕНО» (жёлтая заливка +
  красный текст) через `MarkNotFound` во всех 4 ветках; добавлен TC-69 (SKIP с обоснованием,
  закрыт бизнес-прогоном). Регрессия P2: Total=70/Passed=60/Failed=0/Skipped=10; P1-бизнес-тест PASS.
...
- [ ] **G-CHAT-UI** · глобальная (активная, v1.1.6.2) · независимая · условие: диалог по этапам — постепенная интеграция SourceCraft `chat-interface` для VSCode (https://sourcecraft.dev/portal/docs/ru/code-assistant/operations/agent/chat-interface); этапы: 1) chat-rules (`.codeassistant/rules/`, Задача 1 v1.1.6) — **выполнено**, 2) skills (Задача 2 v1.1.6) — **отложено**, 3) chat-interface (VSCode) — не начато; полный объём каждого этапа уточняется при переходе
