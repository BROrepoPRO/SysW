...
> **Версия системы:** 1.0.12
> **Дата:** 2026-08-26
> **Статус:** Актуализирован (v1.0.12: универсальный поиск листов — имя группы из `main!$B$14`, нейтральные имена `Btn_Search_*`/`Btn_ClearFilter`; ручной подбор запчастей `PickParts_UI`; удалены устаревшие макросы; обновлены скрипты и шаблоны `base/templates/`; тесты расширены до TC-58)

---

## 1. Текущее состояние

Система SysW (v1.0.12) — модульная VBA-надстройка для Excel с обвязкой на Python/PowerShell, автоматизирующая обработку заказ-нарядов авторемонта (импорт данных из отчётов, заполнение шапки заказа, поиск по ГРЗ, учёт работ и запчастей).
...
| **Тестирование** | ✅ v0.2.0, актуализировано v1.0.12 | `Mod_FullTestRunner` — TC-01..TC-58 + TC-S1..TC-S3 (полный перечень — см. `docs/reestr.md`, раздел 3) |
| **Техническая документация** | ✅ v0.5.0 | `docs/DEVELOPER.md` — архитектура, кодировка, CI/CD, тестирование |
| **Реестр макросов, скриптов и тестов** | ✅ v1.0.10 | [`docs/reestr.md`](reestr.md) — полный реестр процедур VBA, скриптов автоматизации и тестов с замечаниями аудита |
...
### Отложенные задачи (v1.0.13, deferred)

> Задачи рефакторинга поиска з/ч и работ (плана `plans/plan_1.0.13_refactor_search.md`)
> реализованы и закоммичены локально (коммит `c9fb3d1`, ветка `dev`), но часть шагов
> заблокирована внешними условиями (сеть/Excel) и отложена.

- [ ] **R-PUSH** · отложенная · независимая · условие: доступ к github.com (сеть/VPN) — выполнить `git push origin dev` (коммит `c9fb3d1`); после успешного пуша — слияние `dev → main` по подтверждению пользователя и возврат на `dev`
- [ ] **R-Z4GEN** · отложенная · независимая · условие: наличие `SysW.db` (после миграции) — запустить `python scripts/build_global_parts.py` для генерации `base/models/z4.xlsx` (650 000+ позиций)
- [ ] **R-TESTEXCEL** · отложенная · независимая · условие: доступ к Excel/COM — полный прогон тестов (`run_tests.py`) включая новые TC-60..62 (компиляция уже проверена `check_vba_syntax.py`)
