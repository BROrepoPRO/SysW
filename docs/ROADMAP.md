...
> **Версия системы:** 1.0.7
> **Дата:** 2026-08-21
> **Статус:** Актуализирован (v1.0.7: активирована глубокая подстановка модельных кодов, реализован единый конвейер `scripts/build_all.py`, SQLite реализован в `SysW.db`, обновлены планы и документация)

---

## 1. Текущее состояние

Система SysW (v1.0.7) — модульная VBA-надстройка для Excel с обвязкой на Python/PowerShell, автоматизирующая обработку заказ-нарядов авторемонта (импорт данных из отчётов, заполнение шапки заказа, поиск по ГРЗ, учёт работ и запчастей).
