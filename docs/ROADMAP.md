...
Система SysW (v1.0.10) — модульная VBA-надстройка для Excel с обвязкой на Python/PowerShell, автоматизирующая обработку заказ-нарядов авторемонта (импорт данных из отчётов, заполнение шапки заказа, поиск по ГРЗ, учёт работ и запчастей).
...
| **Тестирование** | ✅ v0.2.0, актуализировано v1.0.10 | `Mod_FullTestRunner` — TC-01..TC-50 + TC-S1..TC-S3 (полный перечень — см. `docs/reestr.md`, раздел 3) |
