...
> **Версия системы:** 1.1.5
> **Дата:** 2026-08-26
> **Статус:** Актуализирован (**v1.1.4**): интеграция точного описания режимов SourceCraft
> Code Assistant (`Code`/`Ask`/`Architect`/`Debug`/`Orchestrator`) в `.ycarules` и
> `docs/sourcecraft-guide.md`; полная миграция всех групп моделей (`2170`, `2180`, `2190`,
> `4x4`, `GAZ`, `UAZ`) в `matlib_entries` (SysW.db); бизнес-прогон
> `run_oab_reconcile_business_test.py` — **PASS** (PASS=2, FAIL=0). Ранее (v1.1.2):
...
Система SysW (v1.1.4) — модульная VBA-надстройка для Excel с обвязкой на Python/PowerShell, автоматизирующая обработку заказ-нарядов авторемонта (импорт данных из отчётов, заполнение шапки заказа, поиск по ГРЗ, учёт работ и запчастей).
...
- [ ] **G-CHAT-UI** · глобальная (отложенная, v1.1.6) · независимая · условие: диалог по этапам — постепенная интеграция SourceCraft `chat-interface` для VSCode (https://sourcecraft.dev/portal/docs/ru/code-assistant/operations/agent/chat-interface); начальные этапы: 1) chat-rules (`.codeassistant/rules/`, Задача 1 v1.1.6), 2) skills (Задача 2 v1.1.6), 3) chat-interface (VSCode); полный объём каждого этапа уточняется при переходе
