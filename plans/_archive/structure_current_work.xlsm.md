...
### Шапка заказа (столбец B, строки 4–17)

| Ячейка | Поле | Источник |
|--------|------|----------|
| B4 | Номер заказа | Ввод пользователя |
| B5 | № заказа (формат "00{num}-20") | Авто из B4 |
| B6 | Модель | Из spisok (кол. B) |
| B7 | ГРЗ | Из spisok (кол. C) |
| B8 | VIN | Из spisok (кол. D) |
| B9 | Гараж. № | Из spisok (кол. E) |
| B10 | Год выпуска | Из spisok (кол. F) |
| B11 | Пробег | Из spisok (кол. G) |
| B12 | Дата | Из spisok (кол. H) |
| B13 | Цена н/ч | Из model (кол. C) |
| B14 | Группа | Из model (кол. B) | Она же имя файла с модельной базой зч и работ (прим.: UAZ.xlsx)
| B15 |  |заглушка
| B16 |  |заглушка
| B17 |  |заглушка
